...
func newTopkPipeline(opts topkOptions) *topkPipeline {
	return &topkPipeline{
		inputs: opts.Inputs,
		batch: &topkBatch{
			Fields:     exprsToFields(opts.SortBy),
...
func exprsToFields(exprs []physical.ColumnExpression) []arrow.Field {
...
NextInput:
	...
	return p.batch.Compact(memory.DefaultAllocator), nil
}
