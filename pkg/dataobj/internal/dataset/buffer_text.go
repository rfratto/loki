...
// TextBuffer creates a [Buffer] that stores a sequence of strings.
func TextBuffer(opts BufferOptions) Buffer[string] {
	var (
...
func (b *textBuffer) Append(value string) bool {
	// Empty strings are treated as nulls.
	if value == "" {
		return b.AppendNull()
	}

	if b.EstimatedSize()+len(value) > b.opts.PageSizeHint {
		return false
	}
...
func (b *textBuffer) AppendNull() bool {
	...
	if err := b.nullsEnc.Encode(0); err != nil {
		panic(fmt.Sprintf("textBuffer.AppendNull: encoding null bitmask entry: %v", err))
	}
	b.rows++
	return true
}

func (b *textBuffer) EstimatedSize() int {
...
func (b *textBuffer) Rows() int {
	return b.rows
}

func (b *textBuffer) Flush() (Page, error) {
	if b.rows == 0 {
