...
/*
func overlapsTimeRange[P Predicate](p TimeRangePredicate[P], minTime, maxTime time.Time) bool {
	switch {
	case p.IncludeStart && p.IncludeEnd:
		return !maxTime.Before(p.StartTime) && !minTime.After(p.EndTime)
	case p.IncludeStart && !p.IncludeEnd:
		return !maxTime.Before(p.StartTime) && minTime.Before(p.EndTime)
	case !p.IncludeStart && p.IncludeEnd:
		return maxTime.After(p.StartTime) && !minTime.After(p.EndTime)
	case !p.IncludeStart && !p.IncludeEnd:
		return maxTime.After(p.StartTime) && minTime.Before(p.EndTime)
	default:
		panic("unreachable")
	}
}
